<?php

/**
 * Title: Q&A
 * Slug: apta/qna
 * Categories: banner
 * Viewport width: 1400
 * Description: A simple question and answer.
 */
?>

<!-- wp:group {"layout":{"type":"constrained"}} -->
<div class="wp-block-group"><!-- wp:heading {"level":3,"fontSize":"large"} -->
    <h3 class="wp-block-heading has-large-font-size"> How many days are there in a week?</h3>
    <!-- /wp:heading -->

    <!-- wp:paragraph -->
    <p>It depends on where you live on the earth, some countries have 4 while others have 8 or more.</p>
    <!-- /wp:paragraph -->

    <!-- wp:separator {"className":"is-style-wide","style":{"spacing":{"margin":{"top":"var:preset|spacing|20","bottom":"var:preset|spacing|20"}}},"backgroundColor":"base-4"} -->
    <hr class="wp-block-separator has-text-color has-base-4-color has-alpha-channel-opacity has-base-4-background-color has-background is-style-wide" style="margin-top:var(--wp--preset--spacing--20);margin-bottom:var(--wp--preset--spacing--20)" />
    <!-- /wp:separator -->
</div>
<!-- /wp:group -->